...
import { StepHeader, ContinueButton, flowAccentColor, C } from '../primitives';
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
          {COMMIT_LEVELS.map((l, i) => (
            <span key={l.label} style={{ fontSize: 10.5, fontWeight: i === idx ? 800 : 500, color: i === idx ? accent : C.t4 }}>{l.label}</span>
          ))}
        </div>
        <motion.div key={idx} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}
          style={{ marginTop: 24, padding: '14px 16px', borderRadius: 13, background: C.surf, border: `1px solid ${C.b1}` }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: accent, textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 6 }}>{intel.split}</div>
