...
// Bands the current→target gap into one of four honest scenarios. Each carries
// its own headline, tone, timeline, and chart milestones.
export function scoreScenario(answers) {
  const { testTrack = 'SAT', currentScore = 1000, targetScore = 1200, gradeIdx = 2 } = answers;
  ...
  if (n <= 0) {
    return {
      ...
      headline: `You're already at ${targetScore} — so let's aim higher.`,
      sub: `Most students who start at your level end up raising their target once they see structured practice pay off.`,
      note: `We'll lock in your ${currentScore} as the floor, not the ceiling — and keep your ${testTrack} sharp while the rest of your application catches up.`,
      milestones: [],
    };
  }
  if (n <= 100) {
  ...
  return {
    band: 'climb', months: g.urgency === 'high' ? 5 : 6, delta,
    headline: `+${delta} points is an ambitious climb — and we've mapped every step.`,
    sub: `We won't pretend a jump like this is easy. Students who make it treat it as a ${g.urgency === 'high' ? '4–5' : '5–6'} month campaign with weekly checkpoints — exactly the plan we'll build for you.`,
    note: g.urgency === 'high'
      ? `As a ${g.label}, your timeline is tight — which is exactly why a week-by-week plan beats cramming.`
      : g.runway,
    milestones: [{ f: 0.3, score: mid(0.35), label: 'Month 2' }, { f: 0.62, score: mid(0.68), label: 'Month 4' }],
  };
}
...
// ── Pace forecast ────────────────────────────────────────────────────────────
// The projection screen after choosing a pace: real numbers, adapted to their
// pace, test timeline, and score gap.
export function paceForecast(answers) {
  const { testTrack = 'SAT', currentScore = 1000, targetScore = 1200, speedLevel = 1, testTimeline } = answers;
  ...
  return {
    months,
    headline: urgentTest ? 'Your test is close — here\'s your sprint.' : 'Here\'s what your pace unlocks.',
    sub: urgentTest
      ? `With your ${testTrack} inside 3 months, we'll front-load your highest-impact weaknesses first — the fastest points on the board.`
      : `At this pace, students starting near ${currentScore} typically reach ${targetScore} in about ${months} month${months === 1 ? '' : 's'}.`,
    currentScore, targetScore, testTrack,
  };
}
