  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', ...R({ gap: 13, flexWrap: 'wrap' }) }}>
        <div style={{
          width: m ? 44 : 50, height: m ? 44 : 50, borderRadius: 15, flexShrink: 0,
        ...
        <div style={{ flex: 1, minWidth: 180 }}>
          <div style={R({ gap: 7, flexWrap: 'wrap' })}>
            <span style={{ fontSize: 10, fontWeight: 800, color: color, letterSpacing: '.1em', textTransform: 'uppercase' }}>Quest</span>
          ...
          <div style={{ fontSize: m ? 15 : 16.5, fontWeight: 800, color: C.t1, fontFamily: C.FD, letterSpacing: '-.02em', marginTop: 3 }}>{assignment.title}</div>
          ...
          <div style={{ ...R({ justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }), marginTop: 5 }}>
        ...
        <div style={{ ...CC({ gap: 7 }), flexShrink: 0, alignItems: 'stretch' }}>
          {ev.done ? (
            <motion.button
              whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
              onClick={() => onClaim?.(assignment)} disabled={busyId === assignment.id}
              style={{ ...btn(`linear-gradient(135deg, ${C.green}, ${C.greenL})`, { fontSize: 12.5, padding: '9px 16px', color: onTint(C.green) }), opacity: busyId === assignment.id ? 0.6 : 1 }}
            ><Gift size={13} />Claim +{ev.xp}</motion.button>
          ) : (
            <button
              onClick={() => onGo?.(dest)}
              style={btn(`linear-gradient(135deg, ${color}, ${tint(color, 0.7)})`, { fontSize: 12, padding: '8px 15px', color: onTint(color) })}
            >{dest.label}<ChevronRight size={12} /></button>
          )}
          <button onClick={onOpenBoard} style={btnG({ fontSize: 11, padding: '6px 12px' })}>
      ...
      {stats.claimable > 1 && (
        <div style={{ position: 'relative', marginTop: 11, fontSize: 11, color: C.greenL, fontWeight: 600 }}>
