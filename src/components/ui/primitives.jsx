    ...
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        ...
        <circle
          ...
          style={{ transition: 'stroke-dashoffset 360ms cubic-bezier(.16,1,.3,1)', filter: `drop-shadow(0 0 4px ${color}80)` }}
        />
      </svg>
      {label && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontSize: size > 60 ? 14 : 10, fontWeight: 700, color, fontFamily: C.FM, lineHeight: 1 }}>{label}</span>
          {sub && <span style={{ fontSize: 9, color: C.t3, lineHeight: 1, marginTop: 4 }}>{sub}</span>}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ height: h, background: 'rgba(255,255,255,0.06)', borderRadius: h, overflow: 'hidden' }}>
      <div style={{
        ...
        // scaleX, not width: a width transition relayouts the page on every
        // frame, and progress bars are on screen exactly when the device is
        // busiest. transformOrigin pins the growth to the left edge.
        transform: `scaleX(${Math.min(100, Math.max(0, pct)) / 100})`, transformOrigin: 'left',
        borderRadius: h, transition: 'transform 200ms cubic-bezier(.16,1,.3,1)',
...
export function Stat({ label, value, icon, color = C.blue, sub, onClick, m = false }) {
  return (
    <div onClick={onClick} style={{ ...glass({ padding: m ? 16 : 20 }), position: 'relative', overflow: 'hidden', cursor: onClick ? 'pointer' : undefined, transition: CONTROL_TRANSITION }}>
      ...
      <div style={R({ gap: m ? 10 : 12, alignItems: 'flex-start' })}>
        <div style={{ width: m ? 32 : 36, height: m ? 32 : 36, borderRadius: 8, background: `${color}18`, border: `1px solid ${color}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', color, flexShrink: 0, boxShadow: `0 4px 12px ${color}20` }}>{icon}</div>
