...
export default function DebriefCard({ station, answer, result, accent = C.violet }) {
  ...
  return (
    <div style={CC({ gap: 0 })}>
      ...
      {result ? (
        <>
          <div style={{ ...glass2({ padding: 16 }), background: `linear-gradient(135deg, ${tint(accent, 0.06)}, transparent 60%)` }}>
            <div style={R({ gap: 8, marginBottom: 8, justifyContent: 'space-between', flexWrap: 'wrap' })}>
              <div style={R({ gap: 8 })}>
                <Sparkles size={13} color={accent} />
                <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))', color: accent }}>Rater feedback</span>
              </div>
              <span style={pill(tint(accent, 0.14), accent, { fontSize: 10 })}>{result.anchor.label}</span>
            </div>
            <div style={{ fontSize: 12.5, color: C.t3, lineHeight: 1.55, marginBottom: 8 }}>{result.anchor.blurb}</div>
            <div style={{ fontSize: 13, color: C.t1, lineHeight: 1.55, whiteSpace: 'pre-wrap' }}>{result.text}</div>
          </div>

          {result.reasons?.length > 0 && (
            <Block icon={TriangleAlert} title="What to work on next" color={C.amberL}>
              <ul style={{ margin: 0, paddingLeft: 16, display: 'flex', flexDirection: 'column', gap: 4 }}>
                {result.reasons.map((r, i) => <li key={i} style={{ fontSize: 12, color: C.t2, lineHeight: 1.55 }}>{r}</li>)}
              </ul>
            </Block>
          )}
          ...
          {result.strengths?.length > 0 && (
            <Block icon={Target} title="What actually worked" color={C.greenL}>
              <ul style={{ margin: 0, paddingLeft: 16, display: 'flex', flexDirection: 'column', gap: 4 }}>
                {result.strengths.map((r, i) => <li key={i} style={{ fontSize: 12, color: C.t2, lineHeight: 1.55 }}>{r}</li>)}
              </ul>
            </Block>
          )}
        </>
      ...
      <Block icon={Ear} title="What the rater was listening for">
        <ul style={{ margin: 0, paddingLeft: 16, display: 'flex', flexDirection: 'column', gap: 4 }}>
          {station.raterLooksFor.map((r, i) => <li key={i} style={{ fontSize: 12, color: C.t2, lineHeight: 1.55 }}>{r}</li>)}
        </ul>
      </Block>

      <Block icon={TriangleAlert} title="How this station is usually failed" color={C.rose}>
        <div style={{ fontSize: 12, color: C.t2, lineHeight: 1.55 }}>{station.commonFailure}</div>
      </Block>
      ...
      {station.actor && (
        <Block icon={Users} title="What the actor was playing">
          <div style={{ fontSize: 12, color: C.t2, lineHeight: 1.55 }}>
            ...
            <div style={{ marginTop: 4 }}><strong style={{ color: C.t1 }}>What they wanted: </strong>{station.actor.wants}</div>
            <div style={{ marginTop: 4 }}><strong style={{ color: C.roseL }}>What made it worse: </strong>{station.actor.escalates}</div>
            <div style={{ marginTop: 4 }}><strong style={{ color: C.greenL }}>What opened them up: </strong>{station.actor.softens}</div>
          </div>
        </Block>
      )}
      ...
      {station.task && (
        <Block icon={ClipboardList} title="What your partner was holding">
          <div style={{ fontSize: 12, color: C.t2, lineHeight: 1.55 }}>
            ...
            {station.task.twist && <div style={{ marginTop: 4 }}><strong style={{ color: C.amberL }}>The twist: </strong>{station.task.twist}</div>}
          </div>
        </Block>
      )}
      ...
      {station.followUps?.length > 0 && (
        <Block icon={Ear} title="Where this station was going next">
          <ol style={{ margin: 0, paddingLeft: 16, display: 'flex', flexDirection: 'column', gap: 4 }}>
            {station.followUps.map((f, i) => <li key={i} style={{ fontSize: 12, color: C.t2, lineHeight: 1.55 }}>{f}</li>)}
          </ol>
        </Block>
      )}

      <div style={{ ...glass2({ padding: 12, marginTop: 8 }), borderLeft: `3px solid ${accent}` }}>
        ...
        <div style={{ fontSize: 12, color: C.t2, lineHeight: 1.55 }}>{arch.hinge}</div>
      </div>
    </div>
  );
}
