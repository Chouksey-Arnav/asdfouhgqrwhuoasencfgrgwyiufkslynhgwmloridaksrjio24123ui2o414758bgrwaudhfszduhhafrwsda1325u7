...
function TimelineBar({ phases, color, maxYears }) {
  return (
    <div style={{ display: 'flex', gap: 2, height: 10, borderRadius: 5, overflow: 'hidden', background: C.b1 }}>
      {phases.map((p) => {
    ...
    <div style={{
      ...
      >
        <div style={R({ gap: 12, flexWrap: 'wrap', alignItems: 'flex-start' })}>
          <div style={{ flex: 1, minWidth: 190 }}>
            ...
            <div style={{ fontSize: 11, color: C.t3, marginTop: 3 }}>{row.credential}</div>
      ...
      {expanded && (
        <div style={{ padding: '0 14px 14px', borderTop: `1px solid ${C.b1}`, paddingTop: 12, ...CC({ gap: 10 }) }}>
          <p style={{ fontSize: 12.5, color: C.t1, lineHeight: 1.6, margin: 0, fontWeight: 600 }}>{row.headline}</p>
          <p style={{ fontSize: 12, color: C.t2, lineHeight: 1.65, margin: 0 }}>{row.tradeoff}</p>
          ...
          <div style={{ ...glass2({ padding: 11 }), background: C.s2 }}>
            ...
            <div style={{ fontSize: 11.5, color: C.t2, lineHeight: 1.6, marginTop: 8 }}>
          ...
          <p style={{ fontSize: 11.5, color: C.t3, lineHeight: 1.6, margin: 0 }}>
            {audience === 'parent'
...
function Figure({ label, value, sub, color }) {
  return (
    <div style={{ minWidth: 0 }}>
      <div style={{ fontSize: 9.5, fontWeight: 700, color: C.t3, letterSpacing: '.07em', textTransform: 'uppercase' }}>{label}</div>
      <div style={{ fontSize: 16, fontWeight: 800, color, fontFamily: C.FM, marginTop: 2, lineHeight: 1.15 }}>{value}</div>
