  ...
  if (error) {
    return (
      <div style={{ ...glass2({ padding: 16 }), border: `1px solid ${tint(C.rose, 0.3)}`, background: C.roseDim }}>
        <div style={R({ gap: 8, color: C.roseL, fontSize: 12.5, marginBottom: 8 })}>
          <AlertTriangle size={14} />{error}
        </div>
        <button style={btnG({ fontSize: 12, padding: '8px 12px' })} onClick={onRun} disabled={disabled}>
      ...
      <div style={{ ...glass2({ padding: 16 }), border: `1px solid ${tint(C.violet, 0.22)}` }}>
        <div style={CC({ gap: 8 })}>
          ...
          <div>
            <button
              style={{
                ...btnSm(tint(C.violet, 0.16), { color: C.violetL, border: `1px solid ${tint(C.violet, 0.3)}`, fontSize: 12.5, padding: '8px 16px', opacity: disabled ? 0.5 : 1, cursor: disabled ? 'default' : 'pointer' }),
              }}
    ...
    <div style={{ ...glass2({ padding: 16 }), border: `1px solid ${tone.border}` }}>
      <div style={R({ gap: 8, justifyContent: 'space-between', flexWrap: 'wrap', marginBottom: 12 })}>
        <div style={R({ gap: 8 })}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, color: C.violetL, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))' }}>{title}</span>
        </div>
        {critique.score != null && (
      ...
      {critique.headline && (
        <div style={{ fontSize: 13.5, fontWeight: 700, color: C.t1, lineHeight: 1.55, marginBottom: 12, paddingLeft: 12, borderLeft: `3px solid ${tone.fg}` }}>
          {critique.headline}
        </div>
      )}

      <div style={{ fontSize: 12.5, color: C.t2, lineHeight: 1.55 }} dangerouslySetInnerHTML={{ __html: renderMarkdown(critique.body) }} />

      <div style={R({ gap: 8, justifyContent: 'space-between', flexWrap: 'wrap', marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.b1}` })}>
