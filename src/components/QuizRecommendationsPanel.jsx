// ─────────────────────────────────────────────────────────────────────────────
// Metabrain Quiz Recommendations — ranked "what should I do next" panel.
...
// an optional one-tap "Ask Metabrain why" narration powered by the same Groq
 ...
 * @param {Function} [onAskMetabrain] async (pick) => string — optional Groq narration for #1
 */
export default function QuizRecommendationsPanel({ ranked, onStart, onAskMetabrain, compact = false }) {
  ...
  async function askMetabrain(pick) {
    if (!onAskMetabrain || brainLoading) return;
    setBrainLoading(true);
    try {
      const text = await onAskMetabrain(pick);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontSize: 10, fontWeight: 700, color: C.t3, letterSpacing: '.1em', textTransform: 'uppercase' }}>Metabrain Picks — Ranked For You</span>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'stretch', minWidth: 140 }}>
            ...
            {onAskMetabrain && (
              <button
                ...
                onClick={() => askMetabrain(top)}
              >
                {brainLoading ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}><Loader2 size={11} className="spin" /> Thinking…</span> : <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}><Sparkles size={11} />Ask Metabrain why</span>}
              </button>
            )}
          </div>
        </div>
