...
function FieldRow({ label, field, copyValue, mono = false }) {
  ...
  return (
    <div style={{ padding: '8px 0px', borderBottom: `1px solid ${C.b1}` }}>
      <div style={R({ gap: 8, justifyContent: 'space-between', marginBottom: 4, flexWrap: 'wrap' })}>
        <span style={{ fontSize: 10, fontWeight: 700, color: C.t3, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))' }}>{label}</span>
        <div style={R({ gap: 8 })}>
          {field?.limit != null && (
  ...
  return (
    <div style={{ ...CC({ gap: 4 }), marginTop: 8 }}>
      {problems.map((p, i) => {
        ...
        return (
          <div key={i} style={{ ...R({ gap: 8, alignItems: 'flex-start', padding: '8px 8px' }), borderRadius: 8, background: tint(c, 0.07), border: `1px solid ${tint(c, 0.18)}` }}>
...
function SlotCard({ slot, onMove, canUp, canDown, onCategoryChange }) {
  ...
  return (
    <div style={{ ...glass2({ padding: 12 }), borderLeft: `3px solid ${edge}`, background: `linear-gradient(120deg,${tint(edge, 0.045)},rgba(255,255,255,0.02) 55%)` }}>
      <div style={R({ gap: 8, justifyContent: 'space-between', flexWrap: 'wrap' })}>
        <button
          type="button" onClick={() => setOpen(o => !o)}
          style={{ ...R({ gap: 8, flex: 1, minWidth: 160 }), background: 'none', border: 'none', padding: 0, cursor: 'pointer', textAlign: 'left', font: 'inherit' }}
        >
          <span style={{
            width: 24, height: 24, borderRadius: 8, background: tint(edge, 0.15), border: `1px solid ${tint(edge, 0.3)}`,
        ...
        <div style={R({ gap: 4, flexShrink: 0 })}>
          <button type="button" disabled={!canUp} onClick={() => onMove(-1)} style={btnSm(C.s3, { padding: '4px 8px', opacity: canUp ? 1 : 0.3, cursor: canUp ? 'pointer' : 'default' })} aria-label="Move up"><ChevronUp size={12} /></button>
          <button type="button" disabled={!canDown} onClick={() => onMove(1)} style={btnSm(C.s3, { padding: '4px 8px', opacity: canDown ? 1 : 0.3, cursor: canDown ? 'pointer' : 'default' })} aria-label="Move down"><ChevronDown size={12} /></button>
          <button type="button" onClick={() => setOpen(o => !o)} style={btnSm(C.s3, { padding: '4px 8px' })} aria-label={open ? 'Collapse' : 'Expand'}><ChevronDown size={12} style={{ transform: open ? 'rotate(180deg)' : 'none', transition: 'transform .15s' }} /></button>
        </div>
      </div>
      ...
      {open && (
        <div style={{ marginTop: 8 }}>
          <div style={{ padding: '8px 0px', borderBottom: `1px solid ${C.b1}` }}>
            <div style={R({ gap: 8, justifyContent: 'space-between', marginBottom: 4, flexWrap: 'wrap' })}>
              <span style={{ fontSize: 10, fontWeight: 700, color: C.t3, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))' }}>Activity type (the form's dropdown)</span>
              <CopyButton value={slot.category} small />
            </div>
            <select
              value={slot.category}
              onChange={e => onCategoryChange(e.target.value)}
              style={{ ...inp(), fontSize: 12, padding: '8px 8px' }}
            >
              {CA_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            {slot.alternates.length > 0 && (
              <div style={{ fontSize: 10.5, color: C.t4, marginTop: 4, lineHeight: 1.5 }}>
          ...
          <div style={G(2, 10, { marginTop: 8 }, true)}>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: C.t3, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))', marginBottom: 4 }}>Grade levels</div>
              <div style={{ fontSize: 12.5, color: slot.grades.length ? C.t1 : C.t4 }}>{slot.grades.length ? slot.grades.join(', ') : 'none ticked — required'}</div>
            </div>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: C.t3, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))', marginBottom: 4 }}>Timing of participation</div>
              <div style={{ fontSize: 12.5, color: C.t1 }}>{slot.timing}</div>
            </div>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: C.t3, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))', marginBottom: 4 }}>Hours per week</div>
              <div style={{ fontSize: 12.5, color: C.t1, fontFamily: C.FM }}>{slot.hours}</div>
            </div>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: C.t3, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))', marginBottom: 4 }}>Weeks per year</div>
        ...
        <>
          ...
          <motion.div
            ...
            style={isMobile ? {
              ...
              borderRadius: 16, display: 'flex', flexDirection: 'column', boxShadow: '0 24px 70px rgba(0,0,0,0.6)',
            }}
          >
            {/* Header */}
            <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.b1}`, flexShrink: 0, background: `linear-gradient(120deg,${tint(C.blue, 0.1)},rgba(255,255,255,0.02))` }}>
              <div style={R({ gap: 8, justifyContent: 'space-between', flexWrap: 'wrap' })}>
                <div style={R({ gap: 8 })}>
                  <div style={{ width: 32, height: 32, borderRadius: 8, background: `linear-gradient(135deg,${C.blue},${C.violet})`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.t3, padding: 4 }} aria-label="Close"><X size={18} /></button>
              </div>

              <div style={R({ gap: 4, marginTop: 12, flexWrap: 'wrap' })}>
              ...
              <div style={{ fontSize: 10.5, color: C.t4, marginTop: 8, lineHeight: 1.5 }}>{activeFormat.hint}</div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
              {format === 'commonapp' ? (
                <div style={CC({ gap: 12 })}>
                  {exported.errors > 0 && (
                    <div style={{ ...R({ gap: 8, alignItems: 'flex-start', padding: '12px 12px' }), borderRadius: 8, background: tint(C.rose, 0.08), border: `1px solid ${tint(C.rose, 0.22)}` }}>
                      <AlertTriangle size={14} color={C.roseL} style={{ flexShrink: 0, marginTop: 4 }} />
                  ...
                  <div style={{ ...R({ gap: 8, alignItems: 'flex-start', padding: '12px 12px' }), borderRadius: 8, background: tint(C.blue, 0.06), border: `1px solid ${tint(C.blue, 0.16)}` }}>
                    <Info size={13} color={C.blueL} style={{ flexShrink: 0, marginTop: 4 }} />
                  ...
                  {exported.overflow.length > 0 && (
                    <div style={{ ...glass2({ padding: 12 }), border: `1px solid ${tint(C.amber, 0.25)}`, background: tint(C.amber, 0.05) }}>
                      ...
                      <div style={{ fontSize: 11.5, color: C.t2, lineHeight: 1.55, marginBottom: 8 }}>
                      ...
                      {exported.overflow.map((s, i) => (
                        <div key={s.id} style={R({ gap: 8, justifyContent: 'space-between', padding: '8px 0px', borderTop: `1px solid ${C.b1}` })}>
                  ...
                  <div style={{ ...glass2({ padding: 12 }), borderLeft: `3px solid ${C.gold}` }}>
                    <div style={R({ gap: 8, marginBottom: 8 })}>
                      <Award size={13} color={C.goldL} />
                      <span style={{ fontSize: 11, fontWeight: 800, color: C.goldL, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))' }}>Honors section — {exported.counts.usedHonors} of {CA_LIMITS.maxHonors} slots</span>
                    </div>
                    {exported.honors.length === 0 ? (
                      ...
                      <div key={h.id} style={{ padding: '8px 0px', borderTop: `1px solid ${C.b1}` }}>
                        <div style={R({ gap: 8, justifyContent: 'space-between', flexWrap: 'wrap' })}>
                ...
                <pre style={{
                  margin: 0, fontFamily: C.FM, fontSize: 11.5, color: C.t2, lineHeight: 1.55,
                  whiteSpace: 'pre-wrap', wordBreak: 'break-word',
                  background: 'rgba(255,255,255,0.03)', border: `1px solid ${C.b1}`, borderRadius: 8, padding: 12,
                }}>{plainText}</pre>
              )}
            </div>

            {/* Footer */}
            <div style={{ padding: '12px 20px', borderTop: `1px solid ${C.b1}`, flexShrink: 0, ...R({ gap: 8, flexWrap: 'wrap', justifyContent: 'space-between' }) }}>
              ...
              <div style={R({ gap: 8, flexWrap: 'wrap' })}>
                {onExportPdf && (
                  <button style={btnG({ fontSize: 12, padding: '8px 12px' })} onClick={onExportPdf}><FileDown size={13} />Résumé PDF</button>
                )}
                <button
                  style={{ ...btn(`linear-gradient(135deg,${C.blue},${C.violet})`, { fontSize: 12.5, padding: '8px 16px' }) }}
                  onClick={() => {
