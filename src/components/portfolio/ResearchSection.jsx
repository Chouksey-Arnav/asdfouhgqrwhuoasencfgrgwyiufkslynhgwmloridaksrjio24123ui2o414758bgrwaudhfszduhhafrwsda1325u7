    ...
    <div style={CC({ gap: 16 })}>
      ...
      <div style={CC({ gap: 8 })}>
        {entries.map(e => {
          ...
          return (
            <div key={e.id} style={{ ...glass2({ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px' }), borderLeft: `3px solid ${sc}`, background: `linear-gradient(120deg,${tint(sc, 0.05)},rgba(255,255,255,0.02) 55%)` }}>
              <div style={{ width: 34, height: 34, borderRadius: 8, background: tint(accent, 0.13), border: `1px solid ${tint(accent, 0.25)}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><FlaskConical size={15} color={accent} /></div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: C.t1, fontFamily: C.FD }}>{e.title}</div>
                <div style={{ fontSize: 11, color: C.t3, marginTop: 4 }}>{[e.mentor_name, e.institution].filter(Boolean).join(' · ')}{e.hours ? ` · ${e.hours}h` : ''}</div>
