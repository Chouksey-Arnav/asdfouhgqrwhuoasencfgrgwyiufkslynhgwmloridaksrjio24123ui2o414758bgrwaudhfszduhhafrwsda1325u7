...
function BasisChip({ basis, compact = false }) {
  ...
  return (
    <span title={meta.blurb} style={pill(tint(col, 0.13), accentText(col), {
      fontSize: compact ? 8.5 : 9, fontWeight: 800, letterSpacing: '.05em', textTransform: 'uppercase',
...
function StaleWarning({ v, isMobile }) {
  ...
  return (
    <div style={{
      ...R({ gap: 9, alignItems: 'flex-start', padding: isMobile ? '9px 11px' : '10px 13px' }),
      borderRadius: 10, background: tint(col, 0.08), border: `1px solid ${tint(col, 0.28)}`,
    }}>
      {v.stale ? <FileWarning size={13} color={col} style={{ marginTop: 1, flexShrink: 0 }} />
        : <Clock size={13} color={col} style={{ marginTop: 1, flexShrink: 0 }} />}
      <span style={{ fontSize: 11, color: C.t2, lineHeight: 1.6 }}>
  ...
  return (
    <div style={{
      ...
      {(row.note || row.fixWhere) && (
        <div style={R({ gap: 8, flexWrap: 'wrap', marginTop: 6 })}>
          {row.fixWhere && onGoTo && (
            <button type="button" onClick={() => onGoTo(row.fixWhere)}
              style={btnSm('transparent', { fontSize: 10, color: accentText(col), border: `1px solid ${tint(col, 0.3)}`, padding: '4px 9px' })}>
              {row.status === 'short' || row.status === 'missing' ? 'Fix this' : 'Open'}
            </button>
          )}
          {row.note && (
            <button type="button" onClick={() => setOpen(o => !o)} aria-expanded={open}
              style={btnSm('transparent', { fontSize: 10, color: C.t3, border: `1px solid ${C.b1}`, padding: '4px 9px' })}>
      ...
      {open && row.note && (
        <div style={{ fontSize: 11, color: C.t2, lineHeight: 1.65, marginTop: 7, paddingTop: 7, borderTop: `1px solid ${C.b1}`, fontStyle: 'italic' }}>
          {row.note}
        </div>
      )}
    </div>
...
function ProgramCard({ program, facts, saved, saving, onSave, onGoTo, isMobile }) {
  ...
  return (
    <motion.div layout style={{
      ...
      <div style={{ padding: isMobile ? 14 : 17, ...CC({ gap: 11 }) }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: C.t3, marginTop: 3 }}>{program.institution}</div>
        </div>
        ...
        <div style={{
          ...R({ gap: 9, alignItems: 'flex-start', padding: '10px 12px' }),
          borderRadius: 10, background: tint(col, 0.07), border: `1px solid ${tint(col, 0.2)}`,
        }}>
          <Target size={13} color={col} style={{ marginTop: 1, flexShrink: 0 }} />
          <span style={{ minWidth: 0 }}>
            <span style={{ display: 'block', fontSize: 11.5, fontWeight: 800, color: accentText(col) }}>{readiness.headline}</span>
            <span style={{ display: 'block', fontSize: 10.5, color: C.t3, marginTop: 3, lineHeight: 1.55 }}>
        ...
        <div style={R({ gap: 8, alignItems: 'flex-start' })}>
          <GraduationCap size={12} color={C.t4} style={{ marginTop: 2, flexShrink: 0 }} />
          <span style={{ fontSize: 11, color: C.t3, lineHeight: 1.55, minWidth: 0 }}>
            ...
            {accept.note && <span style={{ display: 'block', marginTop: 3 }}>{accept.note}</span>}
          </span>
        </div>

        {policy && (
          <div style={R({ gap: 8, alignItems: 'flex-start' })}>
            <ShieldAlert size={12} color={toneColor(policy.tone)} style={{ marginTop: 2, flexShrink: 0 }} />
        ...
        <div style={{ fontSize: 12, color: C.t2, lineHeight: 1.7 }}>{program.why}</div>

        {/* ── Interview and CASPer prep — the link that was missing ───────── */}
        {interviews.length > 0 && (
          <div style={{ ...CC({ gap: 8, padding: '11px 12px' }), borderRadius: 10, background: tint(C.orange, 0.06), border: `1px solid ${tint(C.orange, 0.2)}` }}>
            <div style={R({ gap: 7, flexWrap: 'wrap' })}>
              <Mic size={12} color={C.orange} />
              <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.09em', textTransform: 'uppercase', color: accentText(C.orange) }}>
                How this one assesses you
              </span>
            </div>
            {interviews.map(t => (
              <div key={t.key} style={CC({ gap: 6 })}>
                ...
                <button type="button" onClick={() => practise(t)}
                  ...
                  <Mic size={11} style={{ marginRight: 5 }} />{t.cta}
                </button>
              </div>
            ))}
          </div>
        ...
        {open && (
          <div style={CC({ gap: 8, paddingTop: 4 })}>
            <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.09em', textTransform: 'uppercase', color: C.t3 }}>
            ...
            {program.supplemental?.prompts?.length > 0 && (
              <div style={{ ...CC({ gap: 6, padding: '11px 12px' }), borderRadius: 10, background: C.surf2, border: `1px solid ${C.b1}` }}>
                <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.09em', textTransform: 'uppercase', color: C.t3 }}>
                  Supplemental prompts
                </span>
                {program.supplemental.prompts.map((p, i) => (
                  <div key={i} style={R({ gap: 7, alignItems: 'flex-start' })}>
                    <span style={{ fontSize: 10, fontFamily: C.FM, color: C.t4, flexShrink: 0, marginTop: 2 }}>{i + 1}</span>
                    <span style={{ fontSize: 11.5, color: C.t2, lineHeight: 1.6 }}>{p}</span>
                  </div>
                ))}
            ...
            {(program.sources || []).length > 0 && (
              <div style={CC({ gap: 5 })}>
                <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.09em', textTransform: 'uppercase', color: C.t3 }}>Sources</span>
                {program.sources.map((s, i) => (
                  <a key={i} href={s.url} target="_blank" rel="noreferrer"
                    style={{ fontSize: 11, color: accentText(C.blue), textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
        ...
        return (
          <div key={m.id} style={{
            ...
            <div style={R({ gap: 8, flexWrap: 'wrap', marginBottom: 6 })}>
              ...
              <span style={pill(tint(col, 0.15), accentText(col), { fontSize: 9, fontWeight: 800, letterSpacing: '.06em', textTransform: 'uppercase' })}>
            ...
            <div style={{ fontSize: 11.5, color: C.t2, lineHeight: 1.7, marginTop: 7 }}>{m.detail}</div>
            <div style={{ fontSize: 11.5, color: C.t2, lineHeight: 1.7, marginTop: 7, paddingTop: 7, borderTop: `1px solid ${C.b1}` }}>
  ...
  return (
    <div style={CC({ gap: 20 })}>
      ...
      <div style={{
        ...
        <div style={R({ gap: 9, marginBottom: 7, flexWrap: 'wrap' })}>
          <ShieldAlert size={14} color={C.amber} />
          <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.1em', textTransform: 'uppercase', color: accentText(C.amber) }}>
            Read this before you read a single acceptance rate
          </span>
        </div>
        <div style={{ fontSize: 12, color: C.t2, lineHeight: 1.75, maxWidth: 760 }}>
      ...
      {soon.length > 0 && (
        <div style={{
          ...
          <div style={R({ gap: 9, marginBottom: 4, flexWrap: 'wrap' })}>
            ...
            <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.1em', textTransform: 'uppercase', color: accentText(C.rose) }}>Closing soonest</span>
          </div>
          <div style={{ fontSize: 11.5, color: C.t3, lineHeight: 1.6, marginBottom: 12, maxWidth: 720 }}>
              ...
              return (
                <div key={program.id} style={{
                  ...
                  <button type="button" disabled={saved || savingId === program.id} onClick={() => save(program, dl)}
                    ...
                    <span style={{ marginLeft: 5 }}>{saved ? 'Tracked' : 'Track it'}</span>
                  </button>
                </div>
              );
            })}
      ...
      {/* ── The catalog ──────────────────────────────────────────────────── */}
      <section aria-label="Combined-degree programs" style={CC({ gap: 14 })}>
        ...
        <div style={CC({ gap: 9 })}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontSize: 10.5, color: C.t3, marginRight: 2 }}>Pathway:</span>
          ...
          <div style={R({ gap: 8, flexWrap: 'wrap' })}>
            <span style={{ ...R({ gap: 6, padding: '7px 11px' }), borderRadius: 9, background: C.surf2, border: `1px solid ${C.b1}`, flex: 1, minWidth: 190 }}>
            ...
            <select value={state} onChange={e => setStateFilter(e.target.value)} aria-label="Filter by state"
              style={{ padding: '7px 11px', borderRadius: 9, background: C.surf2, border: `1px solid ${C.b1}`, color: C.t2, font: 'inherit', fontSize: 12 }}>
        ...
        {loading && !list.length ? (
          ...
          <AnimatePresence initial={false}>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill,minmax(400px,1fr))', gap: 14 }}>
              {list.map(p => (
      ...
      <Disclosure id="combined-degree-roster" icon={Compass} color={C.teal} m={isMobile}
        ...
        <div style={CC({ gap: 12 })}>
          ...
          {roster.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill,minmax(280px,1fr))', gap: 9 }}>
              {roster.map(r => {
                ...
                return (
                  <div key={r.id} style={{ ...glass2({ padding: 12 }), borderLeft: `3px solid ${tint(gc, 0.5)}` }}>
                    ...
                    <div style={{ fontSize: 11, color: C.t2, marginTop: 3, lineHeight: 1.5 }}>{r.program}</div>
                    <div style={R({ gap: 5, flexWrap: 'wrap', marginTop: 7 })}>
                      <span style={pill(tint(gc, 0.12), accentText(gc), { fontSize: 9 })}>{r.degree} · {r.years} yr</span>
                      <span style={pill(C.surf2, C.t3, { fontSize: 9, gap: 3 })}><MapPin size={9} />{r.state}</span>
                    </div>
                  </div>
                );
              })}
