...
// Skills & certifications — formerly the standalone Portfolio → Skills & Certs tab, now a
// section of Activities & Résumé. CPR/BLS/EMT and friends are concrete, dated, verifiable
// credentials, which is exactly why they belong on the same page as the résumé they get
// exported into rather than one tab away from it.
export default function CredentialsSection({ accent = C.teal, entries = [], setEntries, loading = false, onChanged, isMobile = false }) {
  ...
  return (
    <div style={CC({ gap: 18 })}>
      <SectionIntro icon={BadgeCheck} color={accent} color2={C.cyan} m={isMobile}
        ...
        blurb="CPR/BLS, EMT, lifeguard, lab safety — anything with a real certificate and an expiration date. Concrete, checkable credentials rather than self-described skills, and they ride along on every résumé you export from this tab."
          ...
          return (
            <div key={e.id} style={{ ...glass2({ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px' }), borderLeft: `3px solid ${sc}`, background: `linear-gradient(120deg,${tint(sc, 0.05)},rgba(255,255,255,0.02) 55%)`, opacity: expired ? 0.7 : 1 }}>
              ...
              <div style={{ ...R({ gap: 6 }), flexShrink: 0 }}>
                ...
                <button style={btnSm(C.roseDim, { color: C.rose })} onClick={() => removeEntry(e.id)} aria-label="Delete certification"><Trash2 size={12} /></button>
              </div>
            </div>
          );
        })}
