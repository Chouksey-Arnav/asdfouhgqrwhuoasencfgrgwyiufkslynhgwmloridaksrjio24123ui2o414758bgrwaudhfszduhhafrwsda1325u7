    ...
    >
      ...
      >
        <div style={R({ justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' })}>
          <div style={R({ gap: 12, alignItems: 'flex-start' })}>
            <div style={{ width: 46, height: 46, borderRadius: 12, background: C.auroraGrad, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: `0 6px 20px ${C.violet}45` }}>
              <Sparkles size={22} color="#fff" />
            </div>
            <div>
              <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))', color: C.violetL, marginBottom: 4 }}>
                {plan.source === 'ai' ? 'Made just for you · Medabrain' : 'Your personalized plan'}
              </div>
              <div style={{ fontSize: 17, fontWeight: 800, color: C.t1, fontFamily: C.FD, letterSpacing: 'calc(-0.11px + var(--msp-letter-spacing))', lineHeight: 1.2 }}>
          ...
          <div style={R({ gap: 8 })}>
            {stats.map((s, i) => (
              <div key={i} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 16, letterSpacing: 'calc(-0.05px + var(--msp-letter-spacing))', fontWeight: 800, color: s.c, fontFamily: C.FD }}>{s.v}</div>
                <div style={{ fontSize: 8.5, color: C.t3, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))' }}>{s.l}</div>
              </div>
            ))}
        ...
        {!open && (
          <p style={{ fontSize: 12.5, color: C.t2, lineHeight: 1.55, margin: '12px 0px 0px', position: 'relative' }}>{plan.summary}</p>
        )}
      </button>
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div style={{ padding: '0px 20px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              <p style={{ fontSize: 13, color: C.t1, lineHeight: 1.55, margin: 0 }}>{plan.summary}</p>

              {plan.focusAreas?.length > 0 && (
                <Block icon={<Target size={13} color={C.blueL} />} title="What we're focusing on" color={C.blue}>
                  {plan.focusAreas.map((f, i) => (
                    <div key={i} style={{ ...glass2({ padding: '8px 12px' }), display: 'flex', gap: 8, alignItems: 'flex-start', marginBottom: 8, borderLeft: `2px solid ${C.blue}` }}>
                      <span style={{ width: 20, height: 20, borderRadius: 4, background: `${C.blue}22`, color: C.blueL, fontSize: 10, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{i + 1}</span>
                      <div>
                        <div style={{ fontSize: 12.5, fontWeight: 700, color: C.t1 }}>{f.title}</div>
                        {f.why && <div style={{ fontSize: 11, color: C.t3, lineHeight: 1.5, marginTop: 4 }}>{f.why}</div>}
                      </div>
                    </div>
                  ))}
                </Block>
                  ...
                  {plan.milestones.map((m, i) => (
                    <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                        ...
                        {i < plan.milestones.length - 1 && <span style={{ width: 2, flex: 1, background: `${C.violet}30`, marginTop: 4 }} />}
                      </div>
                      <div>
              ...
              {plan.weeklyRhythm?.length > 0 && (
                <Block icon={<CalendarRange size={13} color={C.cyanL || C.cyan} />} title="Your weekly rhythm" color={C.cyan}>
                  <div style={{ ...glass2({ padding: '8px 12px' }), borderLeft: `2px solid ${C.cyan}` }}>
              ...
              {plan.firstWeek?.length > 0 && (
                <Block icon={<Rocket size={13} color={C.greenL} />} title="Start this week" color={C.green}>
                  <div style={{ ...glass2({ padding: '8px 12px' }), borderLeft: `2px solid ${C.green}` }}>
              ...
              {plan.unlockSteps?.length > 0 && (
                <Block icon={<Unlock size={13} color={C.amberL} />} title="Unlock your full day-by-day plan" color={C.amber}>
                  <div style={{ ...glass2({ padding: '8px 12px' }), borderLeft: `2px solid ${C.amber}` }}>
              ...
              {plan.ninetyDayGoal && (
                <div style={{ padding: '12px 12px', borderRadius: 12, background: `linear-gradient(135deg, ${C.amber}14, transparent)`, border: `1px solid ${C.amber}30`, display: 'flex', gap: 8, alignItems: 'flex-start' }}>
