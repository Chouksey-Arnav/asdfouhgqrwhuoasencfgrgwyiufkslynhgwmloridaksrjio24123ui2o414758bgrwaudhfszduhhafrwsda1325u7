...
import { C, glass2, tint, CONTROL_TRANSITION } from '../../lib/theme';
...
export default function SatSkillHeatmap({ masteryMap = {}, onSelect, isMobile = false }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {Object.values(SAT_SECTIONS).map(section => (
        <div key={section.id}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <span style={{ width: 8, height: 8, borderRadius: 4, background: section.color }} />
      ...
      <div style={{ ...glass2({ padding: '8px 12px' }), display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        {[[C.rose, '0-20%'], [C.orange, '20-40%'], [C.amber, '40-60%'], [C.lime, '60-80%'], [C.green, '80%+']].map(([col, lab]) => (
          <span key={lab} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10, color: C.t3 }}>
            <span style={{ width: 10, height: 10, borderRadius: 4, background: tint(col, 0.32), border: `1px solid ${tint(col, 0.7)}` }} />
            {lab}
          </span>
        ))}
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10, color: C.t3 }}>
          <span style={{ width: 10, height: 10, borderRadius: 4, border: `1px dashed ${C.b3}` }} />
          not practiced yet
        </span>
