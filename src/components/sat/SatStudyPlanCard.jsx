  ...
  if (error && !plan) {
    return (
      <div style={{ ...glass2({ padding: 16 }), borderColor: tint(C.amber, 0.25) }}>
        ...
        {onRegenerate && (
          <button onClick={onRegenerate} style={{ ...btnG({ padding: '8px 12px', fontSize: 11.5 }), marginTop: 12 }}>
    ...
    >
      ...
      <div style={{ ...R({ gap: 8, flexWrap: 'wrap' }), marginBottom: 12 }}>
        <span style={pill(tint(accent, 0.16), accent, {
          fontSize: 10, fontWeight: 800, letterSpacing: 'calc(0.4px + var(--msp-letter-spacing))', 
          gap: 4, border: `1px solid ${tint(accent, 0.3)}`,
        })}>
        ...
        {onRegenerate && (
          <button
            ...
            style={{ ...btnG({ padding: '4px 12px', fontSize: 11, marginLeft: 'auto' }) }}
          >
      ...
      <h3 style={{
        ...
        letterSpacing: 'calc(-0.025em + var(--msp-letter-spacing))', margin: 0, lineHeight: 1.3,
      }}>
        {plan.headline}
      </h3>
      <p style={{ fontSize: isMobile ? 12.5 : 13, color: C.t2, lineHeight: 1.55, marginTop: 8, marginBottom: 0, maxWidth: 720 }}>
        {plan.diagnosis}
      </p>
        ...
        <div style={CC({ gap: 8 })}>
          {plan.focus.map((f, i) => (
            ...
            >
              ...
              <div style={{ fontSize: 11.5, color: C.t2, lineHeight: 1.55 }}>{f.why}</div>
              <div style={{ ...R({ gap: 4, alignItems: 'flex-start' }), marginTop: 8 }}>
                <ChevronRight size={12} color={f.color} style={{ marginTop: 4, flexShrink: 0 }} />
                <span style={{ fontSize: 11.5, color: C.t1, lineHeight: 1.55, fontWeight: 600 }}>{f.work}</span>
              </div>
            </motion.button>
          ))}
        </div>
      ...
      {plan.weekly.length > 0 && (
        <div style={{ marginTop: 20 }}>
          ...
          <div style={CC({ gap: 8 })}>
            {plan.weekly.map((w, i) => (
              <div key={i} style={{ ...R({ gap: 12, alignItems: 'flex-start' }), ...glass2({ padding: '12px 12px' }) }}>
                <div style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11.5, color: C.t2, lineHeight: 1.55, marginTop: 4 }}>{w.detail}</div>
                </div>
                {w.minutes && (
      ...
      {plan.pacing && (
        <div style={{ marginTop: 20 }}>
          ...
          <div style={{ fontSize: 12.5, color: C.t2, lineHeight: 1.55, maxWidth: 720 }}>{plan.pacing}</div>
        </div>
      )}
          ...
          <div style={CC({ gap: 8 })}>
            {plan.milestones.map((m, i) => (
              <div key={i} style={R({ gap: 8, alignItems: 'flex-start' })}>
                ...
                <span style={{ fontSize: 12, color: C.t2, lineHeight: 1.55 }}>{m}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{
        ...
        marginTop: 20, paddingTop: 12, borderTop: `1px solid ${C.b1}`,
      }}>
