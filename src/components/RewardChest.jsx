    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: 10, fontWeight: 700, color: C.amberL, letterSpacing: '.14em', textTransform: 'uppercase', marginBottom: 14 }}>{eyebrow}</div>
            ...
            {!revealed ? (
              <motion.div key="closed" exit={{ opacity: 0, scale: .9 }}>
                <motion.div
                  ...
                  style={{ width: 88, height: 88, margin: '0 auto 20px', borderRadius: 20, background: `${C.amber}18`, border: `1.5px solid ${C.amber}45`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 40px ${C.amber}25` }}
                >
                  <Package size={40} color={C.amberL}/>
                </motion.div>
                <div style={{ fontSize: 16, fontWeight: 800, color: C.t1, fontFamily: C.FD, marginBottom: 6 }}>{title}</div>
                <div style={{ fontSize: 12, color: C.t3, marginBottom: 22 }}>Tap to open</div>
                <motion.button
                  whileHover={{ scale: 1.04 }} whileTap={{ scale: .95 }}
                  onClick={handleOpen}
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '10px 24px', borderRadius: 9, border: 'none', background: `linear-gradient(135deg,${C.amber},${C.orange})`, color: '#1a1206', fontWeight: 700, fontSize: 13, fontFamily: C.FB, cursor: 'pointer', boxShadow: `0 4px 16px ${C.amber}40` }}
                >
              ...
              <motion.div key="open" initial={{ opacity: 0, scale: .7 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: 'spring', stiffness: 260, damping: 18 }}>
                <div style={{ width: 88, height: 88, margin: '0 auto 18px', borderRadius: 20, background: `${C.green}18`, border: `1.5px solid ${C.green}45`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 50px ${C.green}30` }}>
                  {cosmetic?.Icon ? <cosmetic.Icon size={40} color={cosmetic.color || C.greenL}/> : <Sparkles size={40} color={C.greenL}/>}
                </div>
                {xp > 0 && <div style={{ fontSize: 28, fontWeight: 800, fontFamily: C.FM, color: C.amberL, marginBottom: 4 }}>+{xp} XP</div>}
                {cosmetic && <div style={{ ...pill(C.violetDim, C.violetL, { fontSize: 11 }), marginBottom: 6 }}>New: {cosmetic.name}</div>}
                <div style={{ fontSize: 12, color: C.t3, marginBottom: 22 }}>{cosmetic ? 'Added to your profile' : 'Added to your total'}</div>
                <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: .95 }} onClick={handleClose} style={btn(C.blueGrad, { fontSize: 13, padding: '9px 22px' })}>Nice!</motion.button>
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
