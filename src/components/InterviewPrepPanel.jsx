  ...
  return (
    <div style={CC({ gap: 22 })}>
      ...
      <div style={glass()}>
        ...
        {mode === 'mmi' && (
          <div style={CC({ gap: 9, marginBottom: 14 })}>
            <div style={R({ gap: 6, flexWrap: 'wrap', alignItems: 'center' })}>
              <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.07em', textTransform: 'uppercase', color: C.t3, minWidth: 62 }}>Format</span>
            ...
            <div style={R({ gap: 6, flexWrap: 'wrap', alignItems: 'center' })}>
              <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.07em', textTransform: 'uppercase', color: C.t3, minWidth: 62 }}>Situation</span>
            ...
            {archetypeFilter !== 'all' && (
              <div style={{ fontSize: 11.5, color: C.t3, lineHeight: 1.65 }}>{ARCHETYPES[archetypeFilter].blurb}</div>
            )}
          </div>
        )}

        <AnimatePresence mode="wait">
          <motion.div key={`${qIdx}-${setKey}-${mode}-${insideRoom}`} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ marginBottom: 14 }}>
            {mode === 'standard' && (
              <div style={glass2({ padding: 18 })}>
                <div style={R({ gap: 8, marginBottom: 8, justifyContent: 'space-between' })}>
                  <div style={R({ gap: 8 })}><Mic size={14} color={modeColor} /><span style={{ fontSize: 11, fontWeight: 700, color: modeColor, textTransform: 'uppercase', letterSpacing: '.06em' }}>Question</span></div>
                  <span style={pill(`${modeColor}18`, modeColor, { fontSize: 10, fontFamily: C.FM })}>{qIdx + 1} / {poolLen}</span>
                </div>
                <div style={{ fontSize: 15, fontWeight: 600, color: C.t1, fontFamily: C.FD, lineHeight: 1.5 }}>{stdQuestion}</div>
              </div>
            )}
            ...
            {mode === 'mmi' && (
              <div style={CC({ gap: 12 })}>
                ...
                {insideRoom
                  ...
                  : (
                    <button onClick={() => setInsideRoom(true)} style={{ ...btn(modeColor), alignSelf: 'flex-start', display: 'inline-flex', alignItems: 'center', gap: 7 }}>
        ...
        <div style={R({ gap: 8, marginBottom: 14, flexWrap: 'wrap' })}>
          ...
          {mode === 'mmi' && (
            <button style={{ ...btnG({ fontSize: 12, borderColor: `${C.violet}40`, color: C.violetL }), display: 'inline-flex', alignItems: 'center', gap: 6 }} onClick={() => switchMode('circuit')}>
        ...
        {(mode === 'standard' || insideRoom) && (<>
          ...
          <div style={R({ gap: 10, marginTop: 14, flexWrap: 'wrap' })}>
            <button style={{ ...btn(modeColor, { fontSize: 13 }), display: 'inline-flex', alignItems: 'center', gap: 6, opacity: loading || !answer.trim() ? 0.6 : 1, boxShadow: `0 4px 16px ${modeColor}40` }} disabled={loading || !answer.trim()} onClick={submit}>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ ...glass({ padding: 20 }), background: `linear-gradient(135deg, ${t.dim}, transparent)`, border: `1px solid ${t.main}30` }}>
              <div style={R({ gap: 8, marginBottom: 8, justifyContent: 'space-between', flexWrap: 'wrap' })}>
                <div style={R({ gap: 8 })}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 700, color: t.main, textTransform: 'uppercase', letterSpacing: '.06em' }}>Rater Feedback</span>
                  <span style={pill(C.s3, C.t3, { fontSize: 9.5 })}><GraduationCap size={9} style={{ marginRight: 3 }} />practice</span>
                </div>
                <div style={R({ gap: 8 })}>
              ...
              {mode === 'mmi' && (
                <div style={{ marginTop: 16 }}>
                  <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.08em', textTransform: 'uppercase', color: C.t3, marginBottom: 4 }}>Debrief</div>
