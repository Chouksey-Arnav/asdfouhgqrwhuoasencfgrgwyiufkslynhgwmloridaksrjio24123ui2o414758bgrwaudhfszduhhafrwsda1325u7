...
/**
 * AI, search, and ad crawlers to explicitly welcome onto the public surface,
 * over and above the blanket `User-agent: *` block. None of these need
 * special treatment — they already match `Allow: /$` plus the per-route
 * `Allow` lines under `*` — but an explicit block removes any ambiguity for
 * an operator (or model) skimming robots.txt for "is this bot allowed",
 * which is exactly the kind of audit the search-engine-optimization-seo
 * playbook calls for. Same Allow/Disallow shape as the `*` block, generated
 * by buildRobots() below.
 *
 * Grouped by who operates them. This list is deliberately broad — the goal
 * is that MedSchoolPrep comes up when a student asks ChatGPT, Gemini, or
 * Claude "what's a good app to prep for med school" — so it covers every
 * publicly documented AI/LLM crawler plus the search and ad bots that decide
 * whether the site is indexed and monetizable at all, not just the handful
 * we expect to matter most.
 *
 * OpenAI:
 *   • GPTBot          — feeds ChatGPT/model training.
 *   • ChatGPT-User     — live-browsing agent, fetches a page a user asked about.
 *   • OAI-SearchBot    — powers ChatGPT's search feature.
 *
 * Anthropic:
 *   • ClaudeBot        — training/index crawler.
 *   • Claude-User       — live-browsing agent (Claude fetching a page on request).
 *   • Claude-SearchBot — powers Claude's search feature.
 *   • anthropic-ai     — legacy/alternate Anthropic UA still seen in the wild.
 *
 * Google:
 ...
 *   • Mediapartners-Google — the actual AdSense crawler; this is the one that
 *                      decides whether a page qualifies to show ads, distinct
 *                      from Googlebot.
 *   • AdsBot-Google / AdsBot-Google-Mobile — Google Ads landing-page checks.
 *
 * Other search engines:
 *   • Bingbot / BingPreview — Microsoft Bing, also feeds Copilot.
 *   • DuckDuckBot / DuckAssistBot — DuckDuckGo search + AI assist.
 *   • YandexBot        — Yandex.
 *   • Baiduspider      — Baidu.
 *
 * Other AI/answer engines and model builders:
 *   • PerplexityBot / Perplexity-User — Perplexity's crawler and live-fetch agent.
 *   • Meta-ExternalAgent / Meta-ExternalFetcher / FacebookBot — Meta's crawlers
 *                      (Llama training + link previews).
 *   • Applebot / Applebot-Extended — Siri/Spotlight and Apple's AI features.
 *   • Amazonbot        — Alexa/Amazon AI.
 *   • Bytespider       — ByteDance (TikTok), trains their models.
 *   • cohere-ai        — Cohere.
 *   • Diffbot          — Diffbot's knowledge-graph crawler, widely resold to AI tools.
 *   • CCBot            — Common Crawl, whose dataset many AI labs train on.
 *   • YouBot           — You.com's answer engine.
 *   • Omgili / Omgilibot — dataset crawler licensed to several AI labs.
 *   • Timpibot         — Timpi's decentralized search index.
 *   • ImagesiftBot      — image/content indexing used by several AI products.
 */
